...
int main(){
    ...
    srand(time(0));
    for(int i = 0; i < 100; i++){
         randnums[i] = rand();//generate random nums to be inserted into table
    }
    hashLP h;//constructor
    h.load1();//load
    h.load2();
    int count = 0;
    bool inserted = false;
    ...
    bool flag6 = false;
    int key= 0;
    for(int i = 0; i < 40000; i++){//loop through file array
        key = h.getkey2(i);//get num from array
        inserted = h.insert(key);//insert 
        if(inserted == true){
            count++;
        }
        
        if(h.getloadfactor(count) >= 0.1 && flag1 == false){//if load factor reaches .1
            flag1 = true;
            high_resolution_clock::time_point t1 = high_resolution_clock::now();//start clock
            for(int j = 0; j < 100; j++){
                h.insert(randnums[j]);//insert rand nums
            }
            high_resolution_clock::time_point t2 = high_resolution_clock::now();//end clock
            duration<double> time_span = duration_cast<duration<double>>(t2 - t1);
            cout << "execution time: " << time_span.count()/100.0 << endl;//calc average of 100 iterations
            //------------------------------------------------------------------
            high_resolution_clock::time_point t3 = high_resolution_clock::now();//repeat for search
            for(int l = 0; l < 100; l++){
                h.search(randnums[l]);
            }
            ...
            //------------------------------------------------------------------
            high_resolution_clock::time_point t5 = high_resolution_clock::now();//repeat for delete
            for(int k = 0;k < 100; k++){
        ...
        if(h.getloadfactor(count) >= 0.2 && flag2 == false){//when load factor reaches .2 repeat
            flag2 = true;
        ...
        if(h.getloadfactor(count) >= 0.5 && flag3 == false){//when load factor reaches .5 repeat
            flag3 = true;
        ...
        if(h.getloadfactor(count) >= 0.7 && flag4 == false){//when load factor reaches .7 repeat
            flag4 = true;
        ...
        if(h.getloadfactor(count) >= 0.9 && flag5 == false){//when load factor reaches .9 repeat
            flag5 = true;
        ...
        if(h.getloadfactor(count) >= 1.0 && flag6 == false){//when load factor reaches 1 repeat stop 
            flag6 = true;
            h.del100();
